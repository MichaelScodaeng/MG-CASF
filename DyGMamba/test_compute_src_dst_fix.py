...
def test_compute_src_dst_method():
    """Test that the compute_src_dst_node_temporal_embeddings method exists and works."""
    try:
        import torch
        import numpy as np
        ...
        if hasattr(dynamic_backbone, 'compute_src_dst_node_temporal_embeddings'):
            print('✅ compute_src_dst_node_temporal_embeddings method exists')
            
            # Create test batch data
            batch_size = 4
            batch_src_node_ids = np.random.randint(0, 100, batch_size)
            batch_dst_node_ids = np.random.randint(0, 100, batch_size)
            ...
            expected_shape = (batch_size, test_config['ccasf_output_dim'])  # Should match output_dim
            ...
            print('❌ compute_src_dst_node_temporal_embeddings method does not exist')
            return False
            
        print('\n🎉 SUCCESS: compute_src_dst_node_temporal_embeddings fix is working!')
        return True
